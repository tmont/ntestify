...
namespace NTestify.Tests {
	...
	//    [TestMethod]
	//    public void Property_getter_should_match_expression() {
	//        var expectation = new Expectation<ClassToMock, int>(x => x.Count);
	//        Expression<Func<ClassToMock, int>> expression1 = x => x.Count;
	//        Ass.That(expectation.Matches(expression1));
	//    }

	//    [TestMethod]
	//    public void Property_setter_should_match_expression() {
	//        Action<ClassToMock> action = x => x.Count = 1;
			


	//        //var expectation = new Expectation<ClassToMock>(x => x.Count);
	//        //Expression<Func<ClassToMock, int>> expression1 = x => x.Count;
	//        //Ass.That(expectation.Matches(expression1));
	//    }
